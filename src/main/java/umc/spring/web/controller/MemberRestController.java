...
public class MemberRestController {
    ...
    @Operation(summary = "진행중인 미션 목록 조회 API", description = "특정 회원이 현재 진행 중인 미션 목록을 조회합니다. 페이징 포함")
    @ApiResponses({
            @ApiResponse(responseCode = "COMMON200", description = "성공"),
            @ApiResponse(responseCode = "MEMBER4001", description = "존재하지 않는 회원"),
            @ApiResponse(responseCode = "COMMON400", description = "page가 1보다 작음")
    })
    public umc.spring.apiPayload.ApiResponse<MemberResponseDTO.MissionProgressListDTO> getProgressingMissions(
            @PathVariable Long memberId,
            @ValidPage(message = "PAGE_INVALID") @RequestParam Integer page
    ) {
        Page<MemberMission> missions = memberQueryService.getOngoingMissions(memberId, page - 1);
        return umc.spring.apiPayload.ApiResponse.onSuccess(MemberConverter.toMissionProgressListDTO(missions));
    }
}
